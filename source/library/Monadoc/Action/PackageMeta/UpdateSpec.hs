...
spec :: Hspec.Spec
spec = Hspec.describe "Monadoc.Action.PackageMeta.Update" . Hspec.around Test.withConnection $ do
  Hspec.it "does not throw an error when the package meta doesn't exist" . Test.runFake $ do
    packageMeta <- Test.arbitrary
    PackageMeta.Update.run packageMeta

  Hspec.it "succeeds when the package meta doesn't need to be updated" . Test.runFake $ do
  ...
  Hspec.it "updates a package meta" . Test.runFake $ do
