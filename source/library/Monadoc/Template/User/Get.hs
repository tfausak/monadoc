...
render ::
  ...
  [Package.Model] ->
  Lucid.Html ()
render context hackageUser rows packages = do
  let route = Route.User . HackageUser.name $ Model.value hackageUser
      title = "User " <> (Witch.into @Text.Text . HackageUser.name $ Model.value hackageUser) <> " :: Monadoc"
