...
selectAll = App.query_ "select * from cronEntry"

selectByGuid :: Guid.Guid -> App.App (Maybe CronEntry.Model)
selectByGuid guid = do
  cronEntries <- App.query "select * from cronEntry where guid = ? limit 1" [guid]
  pure $ Maybe.listToMaybe cronEntries

selectByKey :: CronEntry.Key -> App.App (Maybe CronEntry.Model)
selectByKey key = do
  cronEntries <- App.query "select * from cronEntry where key = ? limit 1" [key]
  pure $ Maybe.listToMaybe cronEntries

selectNext :: Timestamp.Timestamp -> App.App (Maybe CronEntry.Model)
selectNext now = do
  rows <- App.query "select * from cronEntry where runAt <= ? order by runAt asc limit 1" [now]
  pure $ Maybe.listToMaybe rows

selectWithGuid :: App.App [CronEntry.Model]
selectWithGuid = App.query_ "select * from cronEntry where guid is not null"
